...
public interface IProgressTrack : ITrack, ILogWidget, IRanked
{
  ...
  public static SelectMenuOptionBuilder ResolveOption(string moveName = "")
  {
    ...
    else
    {
      option.WithLabel(moveName)
        ...
        .WithValue($"progress-roll:{moveName}");
    }
    return option;
  }

  public static SelectMenuOptionBuilder MarkOption(int addTicks, string alertLabel = "")
  {
    string description = $"Mark {TickString(addTicks)} of progress";
    SelectMenuOptionBuilder option = new SelectMenuOptionBuilder().WithEmote(TickEmoji[Math.Min(BoxSize, addTicks)]).WithValue($"progress-mark:{addTicks},{alertLabel}");
  ...
  public static SelectMenuOptionBuilder RecommitOption(string alertTitle = "")
  {
    SelectMenuOptionBuilder option = new SelectMenuOptionBuilder()
      .WithLabel("Recommit")
      .WithValue($"progress-recommit:{alertTitle}")
      .WithEmote(Emoji["recommit"])
    ;
    if (!string.IsNullOrEmpty(alertTitle))
    {
      option.WithDescription(alertTitle);
    }
    return option;
  }
}